...
try {
	...
	if (!table) {
		...
		await section("doctrine-budget", async () => {
			...
			// 2026-09-17: 8,970 × 1.05 rounds up to a 9,500-character bound.
			const WRITING_ROUTER_BOUND = 9500;
			// 2026-09-17: 9,225 × 1.05 rounds up to a 9,800-character bound.
			const ALL_TAILS_BOUND = 9800;
			// 2026-09-17: the 10,410 deferred-issue maximum rounds up to an 11,000-character bound with five percent reserve.
			const MAXIMAL_BOUND = 11000;
			checkAll(
				"doctrine-budget",
				"portable doctrine budgets cover the routing rule, each representative feature basis, and one maximum-shaped all-feature fixture. The maximum fixture uses all nineteen shipped profiles, draft PRs, writing, two capped worker units, and four capped tools. A measured positive control adds one capped tool and six copies of the largest model row, so budget growth cannot pass vacuously",
				[
					...
					["the whole rule stays under 4700 portable chars with five percent reserve", ruleChars <= 4700 && hasDoctrineReserve(ruleChars, 4700), { portableChars: ruleChars, rawChars: rule.length, rows: rows.length }],
					["...and under 37 lines with five percent reserve", rule.split("\n").length <= 37 && hasDoctrineReserve(rule.split("\n").length, 37), rule.split("\n").length],
					...
					[`...and the whole router-on doctrine is the measured 8970 portable chars and 106 lines, and stays under ${WRITING_ROUTER_BOUND} with five percent reserve`, portable(on).length === 8970 && on.split("\n").length === 106 && portable(on).length <= WRITING_ROUTER_BOUND && hasDoctrineReserve(portable(on).length, WRITING_ROUTER_BOUND), { portable: portable(on).length, raw: on.length, lines: on.split("\n").length }],
					...
					["the six-model fixture is 6647 portable chars and 93 lines without draft publishing", portable(configuredOffDraft).length === 6647 && configuredOffDraft.split("\n").length === 93, { portable: portable(configuredOffDraft).length, lines: configuredOffDraft.split("\n").length }],
					...
					[`writing plus router is the measured 8970 portable chars and 106 lines, and stays under ${WRITING_ROUTER_BOUND} with five percent reserve`, portable(writingRouterOn).length === 8970 && writingRouterOn.split("\n").length === 106 && portable(writingRouterOn).length <= WRITING_ROUTER_BOUND && hasDoctrineReserve(portable(writingRouterOn).length, WRITING_ROUTER_BOUND), { portable: portable(writingRouterOn).length, lines: writingRouterOn.split("\n").length }],
					...
					[`all three tail features are the measured 9225 portable chars and 112 lines, and stay under ${ALL_TAILS_BOUND} with five percent reserve`, portable(writingAllOn).length === 9225 && writingAllOn.split("\n").length === 112 && portable(writingAllOn).length <= ALL_TAILS_BOUND && hasDoctrineReserve(portable(writingAllOn).length, ALL_TAILS_BOUND), { portable: portable(writingAllOn).length, lines: writingAllOn.split("\n").length }],
					["the all-nineteen draft fixture is 8989 portable chars and 106 lines", portable(allDraft).length === 8989 && allDraft.split("\n").length === 106, { portable: portable(allDraft).length, lines: allDraft.split("\n").length }],
					["the all-nineteen draft and writing fixture is 8989 portable chars and 106 lines", portable(allDraftWriting).length === 8989 && allDraftWriting.split("\n").length === 106, { portable: portable(allDraftWriting).length, lines: allDraftWriting.split("\n").length }],
					// Update exact measurements with production wording in the same commit.
					[`the maximum all-feature fixture is the measured 10336 portable chars and 116 lines, and stays within ${MAXIMAL_BOUND} with five percent reserve`, maximalPortable === 10336 && maximal.split("\n").length === 116 && maximalPortable <= MAXIMAL_BOUND && hasDoctrineReserve(maximalPortable, MAXIMAL_BOUND), { portable: maximalPortable, raw: maximal.length, lines: maximal.split("\n").length, profiles: realCandidates.length, units: MAX_EXT.units.length, tools: MAX_EXT.units.reduce((n, unit) => n + unit.tools.length, 0) }],
					[`the draft-PR-disabled maximum fixture is pinned independently at 10317 portable chars and 116 lines, and shares the ${MAXIMAL_BOUND} maximum bound`, maximalNoDraftPortable === 10317 && maximalNoDraft.split("\n").length === 116 && maximalNoDraftPortable <= MAXIMAL_BOUND && hasDoctrineReserve(maximalNoDraftPortable, MAXIMAL_BOUND), { portable: maximalNoDraftPortable, raw: maximalNoDraft.length, lines: maximalNoDraft.split("\n").length, profiles: realCandidates.length, units: MAX_EXT.units.length, tools: MAX_EXT.units.reduce((n, unit) => n + unit.tools.length, 0) }],
					...
					[`the positive control is the measured 11700 portable chars and 123 lines, and exceeds ${MAXIMAL_BOUND} by the larger growth unit`, overBudgetPortable === 11700 && overBudget.split("\n").length === 123 && overBudgetPortable > MAXIMAL_BOUND && overBudgetPortable - MAXIMAL_BOUND >= Math.max(maxModelIncrement.growth, maxToolIncrement), { portable: overBudgetPortable, lines: overBudget.split("\n").length, bound: MAXIMAL_BOUND, growthBeyondBound: overBudgetPortable - MAXIMAL_BOUND, maxModelIncrement, maxToolIncrement }],
			...
			checkAll(
				...
				[
					[`the maximal deferred-issue fixture is the measured 10410 portable chars and 117 lines, and stays within ${MAXIMAL_BOUND} with five percent reserve`, maximalFollowUpPortable === 10410 && maximalFollowUp.split("\n").length === 117 && maximalFollowUpPortable <= MAXIMAL_BOUND && hasDoctrineReserve(maximalFollowUpPortable, MAXIMAL_BOUND), { portable: maximalFollowUpPortable, raw: maximalFollowUp.length, lines: maximalFollowUp.split("\n").length, reserveRequired: Math.ceil(maximalFollowUpPortable * 1.05), bound: MAXIMAL_BOUND }],
				],
			);
		});
	}
		...
		await section("router-warnings", async () => {
			...
			await section("router-warning-text", async () => {
				...
				checkAll("router-field-cap", "the 180-character input cap preserves every current shipped unknown field and non-preferred reason, while truncating a hostile 200-character run", [
					...
					["the boundary reasons and their provider copies remain represented", realReasons.filter((text) => text.length === 180).length === 4, realReasons.map((text) => text.length)],
  ...
  synchronization, caching, batching, and benchmark evidence.`;
	...
	if (!table || !state) {
		...
		await section("profiles-meta", async () => {
			...
			checkAll("profiles-meta", "the base profiles and their Codex copies retain the base-corpus date, the experimental profiles retain their documented observation date, and the whole table is deep-frozen so no consumer can mutate shared data", [
				["PROFILES_AS_OF is the exact base-corpus date", table.PROFILES_AS_OF === "2026-07-29", table.PROFILES_AS_OF],
				["every profile has its exact expected observation date", all.length === 19 && experimentalIds.size === 3 && dateMismatches.length === 0, dateMismatches],
