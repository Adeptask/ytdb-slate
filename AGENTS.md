...
The shipped doctrine uses eleven risk-defined focus areas as workflow triggers. User-approved proofs select gates. Reviewer I supplies one general review floor. Draft pull request publishing is optional.
...
- the shipped workflow contracts. These pin complete safety-floor and focus-table content, the eleven risk definitions, and the marked test-quality definition. They also cover focus states, per-track design entry, approved area removal, marker availability, the Reviewer I row, all four production defect charters and production-cap classes, split prose and licensing charters, lifecycle ownership, retired-role and prefix absence, and unique named headings across all five workflow documents.
- the acceptance policy, in ONE audited mechanism. One canonical acceptance-fact set composes the expectation of each of the **five** protected units: § Focus classes and gates and § Delivery and termination in `docs/track-workflow.md`, the marked `track-acceptance` block in `docs/review-rules.md`, the packet acceptance rule in `docs/user-notes.md`, and the rendered session instructions of `extension/mode.ts`. No unit may take a policy fact from another expected source. Each unit resolves exactly once and fails closed, and a unit that resolves zero or several times reports a policy loss that needs a policy review before any re-anchoring or deletion. Bounded mutations attack each unit, and benign text outside a unit — including text appended after the terminal end marker — must change nothing. Mandatory escalation routing and deferred-work tracking are the two Option A rules, and each has an assertion of its own outside every unit. The acceptance summary in `README.md` stays deliberately outside this roster, because it is orientation text and not a place an execution reads. Its proof-count sentence is still kept truthful.
